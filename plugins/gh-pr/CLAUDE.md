...
> **Local fork** of [`wyattjoh/herdr-plugin-gh-pr`](https://github.com/wyattjoh/herdr-plugin-gh-pr), tracked in this config repo and linked via `scripts/link-local-plugins.sh` (replacing the marketplace install). The only change from upstream is the sidebar glyph: status is shown as a **colored emoji dot** instead of a monochrome symbol, because herdr renders `custom_status` as plain text (ANSI/markup is stripped on normalization) so a colored glyph is the only way to convey status by color.
...
While a refresh is in flight, the pane's glyph is swapped for `⟳` (the PR number is kept), then it settles to a colored status dot: `🟢` open and CI green (passing or no checks), `🟡` open but CI not green yet (failing or pending), `🟣` merged, `🔴` closed. The color-by-status mapping lives in `composeLabel` (`src/label.ts`).
