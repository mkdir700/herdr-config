...
test("composeLabel shows a green dot for an open PR with passing CI", () => {
  expect(composeLabel(123, "pass")).toBe("#123 🟢");
});

test("composeLabel shows a yellow dot for an open PR with failing CI", () => {
  expect(composeLabel(7, "fail")).toBe("#7 🟡");
});

test("composeLabel shows a yellow dot for an open PR with pending CI", () => {
  expect(composeLabel(7, "pending")).toBe("#7 🟡");
});

test("composeLabel shows a green dot for an open PR with no checks", () => {
  expect(composeLabel(7, "none")).toBe("#7 🟢");
});

test("composeLabel shows a purple dot for a merged PR", () => {
  expect(composeLabel(9, "fail", "MERGED")).toBe("#9 🟣");
});

test("composeLabel shows a red dot for a closed PR", () => {
