...
async function currentBranch(cwd: string): Promise<string | null> {
	const inside = await $`git -C ${cwd} rev-parse --is-inside-work-tree`
		.nothrow()
		.quiet();
	if (inside.exitCode !== 0 || inside.stdout.toString().trim() !== "true")
		return null;
	const branch = await $`git -C ${cwd} rev-parse --abbrev-ref HEAD`
		.nothrow()
		.quiet();
...
async function prInfo(
	cwd: string,
	branch: string,
): Promise<PullRequest | null> {
	const out = await $`gh pr view ${branch} --json number,state`
		.cwd(cwd)
		.nothrow()
		.quiet();
	if (out.exitCode !== 0) return null;
...
async function prChecks(cwd: string, branch: string): Promise<Check[]> {
	const out = await $`gh pr checks ${branch} --json bucket`
		.cwd(cwd)
		.nothrow()
		.quiet();
	try {
...
async function reportMetadata(args: string[]): Promise<void> {
	const result = await $`${herdr} pane report-metadata ${args}`
		.nothrow()
		.quiet();
	if (result.exitCode !== 0) {
		throw new Error(
			result.stderr.toString().trim() || "failed to update PR status metadata",
		);
	}
}
...
export async function run(targetPaneId?: string, force = false): Promise<void> {
	...
	if (
		!force &&
		!throttleElapsed(lastCheckMs(pane.paneId), now, THROTTLE_WINDOW_MS)
	)
		return;
